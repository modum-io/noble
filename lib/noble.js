...
Noble.prototype.onDiscover = function (uuid, address, addressType, connectable, advertisement, rssi) {
  console.log("noble: discovered ", advertisement.localName, " with ", advertisement.serviceUuids);
  ...
  if (this._allowDuplicates || !previouslyDiscoverd) {
    console.log("noble: emitting discover for peripheral ", peripheral.pCounter, peripheral.advertisement.localName, peripheral.advertisement.serviceUuids);
...
Noble.prototype.onMtu = function (peripheralUuid, mtu) {
  ...
  if(peripheral) {
    ...
    this.emit('warning', `mtu arrived for ${peripheralUuid} but that peripheral is gone!`);
  }
};
