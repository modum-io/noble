...
function Peripheral (noble, id, address, addressType, connectable, advertisement, rssi) {
  ...
  console.log("instantiated pCounter " + this.pCounter + " for name " + advertisement.localName);
  this.id = id;
...
const connect = function (callback, contextId) {
  ...
  if (this.state === 'connected') {
    this.emit('connect', new Error('Peripheral ' + this.address + ' already connected'));
  } else {
...
const disconnect = function (callback) {
  this.once('disconnect', () => {
    console.log("peripheral ", this.pCounter, " disconnecting");
    if (callback) {
      callback(null);
    }
  });
