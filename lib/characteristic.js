...
function Characteristic (noble, peripheralId, serviceUuid, uuid, properties) {
  this.characteristicCounter = characteristicCounter++;
  console.lag("instantiating char ", this.characteristicCounter, peripheralId, serviceUuid, uuid);
  this._noble = noble;
...
const notify = function (notify, callback) {
  console.lag(`they called notify ${notify} on ${this._peripheralId}:characteristic ${this.characteristicCounter}`);
  this.once('notify', () => {
    if (callback) {
      callback(null);
    }
