...
NobleBindings.prototype._onMessage = function (event) {
  ...
  let data = event.data ? Buffer.from(event.data, 'hex') : null;

  if(data && data.byteLength > 500)
  {
    debugger; // wtf, try again?
    data = event.data ? Buffer.from(event.data, 'hex') : null;
  }
  ...
  if (type === 'stateChange') {
    ...
    console.log("discovered ", advertisement.localName, " with ", advertisement.serviceUuids);
    ...
    this.emit('servicesDiscover', undefined, peripheralUuid, serviceUuids);
