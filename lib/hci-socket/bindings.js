...
NobleBindings.prototype.connect = function (peripheralUuid) {
  ...
  // this guy is going to go to the back of the queue
  this._connectionQueue = this._connectionQueue.filter((elem) => {
    return elem !== peripheralUuid;
  });
  ...
  if (!this._pendingConnectionUuid) {
    ...
    connectTimeout = setTimeout(() => {
      console.log("noble-hci-bindings: connection timeout");
      this.doNextConnectionInQueue();
    }, 10000);
  } else {
...
NobleBindings.prototype.doNextConnectionInQueue = function() {
...
NobleBindings.prototype.onDiscover = function (status, address, addressType, connectable, advertisement, rssi) {
  ...
  if(advertisement.serviceUuids.length <= 0) {
    return;
  }
...
NobleBindings.prototype.onLeConnComplete = function (status, handle, role, addressType, address, interval, latency, supervisionTimeout, masterClockAccuracy) {
  ...
  if (status === 0) {
    ...
    if(this._gatts[uuid]) {
      debugger; // this gatt already exists!
    }
...
NobleBindings.prototype.onServicesDiscovered = function (err, address, serviceUuids) {
  const uuid = address.split(':').join('').toLowerCase();

  this.emit('servicesDiscover', undefined, uuid, serviceUuids);
};
