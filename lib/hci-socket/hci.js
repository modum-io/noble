...
const Hci = function () {
  this.id = hciCounter++;
  console.log("constructed hci ", this.id);
  ...
  if (this._isDevUp !== isDevUp) {
    if (isDevUp) {
      if (this._state === 'poweredOff') {
        console.log("hci " + this.id + " went poweroff - removing all listeners");
...
Hci.prototype.setScanEnabled = function (enabled, filterDuplicates) {
  console.log("hci " + this.id + " told to set scan enabled to " + enabled);
  const cmd = Buffer.alloc(6);
...
Hci.prototype.processCmdCompleteEvent = function (cmd, status, result) {
  if (cmd === RESET_CMD) {
    ...
    console.log("hci " + this.id + " has had its scan thing complete. emitting to " + this.listenerCount('leScanEnableSet') + " listeners");
