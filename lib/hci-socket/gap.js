...
const Gap = function (hci) {
  this.id = gapCounter++;
  this._hci = hci;
  console.log("gap " + this.id + " with hci " + hci.id + " constructed");
...
Gap.prototype.startScanning = function (allowDuplicates) {
  console.log("gap ", this.id + " with hci " + this._hci.id + " been told to start scanning");
...
Gap.prototype.onHciLeScanEnableSet = function (status) {
  ...
  if (this._scanState === 'starting') {
    ...
    console.log("gap " + this.id + " with hci " + this._hci.id + " stopped scanning");
