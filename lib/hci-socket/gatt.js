...
Gatt.prototype.discoverServices = function (uuids) {
  ...
  const callback = (data) => {
    ...
    if (opcode !== ATT_OP_READ_BY_GROUP_RESP || services[services.length - 1].endHandle === 0xffff) {
      ...
      console.log("hci gatt " + this._address + " has " + Object.keys(this._services) + " services");
      this.emit('servicesDiscovered', this._address, JSON.parse(JSON.stringify(services)) /* services */);
      this.emit('servicesDiscover', undefined, this._address, serviceUuids);
    } else {
...
Gatt.prototype.read = function (serviceUuid, characteristicUuid) {
  const characteristic = this._characteristics && 
...
Gatt.prototype.notify = function (serviceUuid, characteristicUuid, notify) {
  const characteristic = this._characteristics && 
                         ...
                         this._characteristics[serviceUuid][characteristicUuid];
  if(!characteristic) {
    console.log("hci-gatt - couldn't find the characteristic you told me about...");
    return; // something funky is going on...
  }
